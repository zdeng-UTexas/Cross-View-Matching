# This script crops image patches from the aerial image based on the GPS coordinates of the ground-level images.
# Usage: python utils/crop_aerial_image_pathches.py --patch_size 32
...
def create_output_directory(size):
    output_dir = os.path.join(datasets_dir, f'aerial_image_patches_{size}')
...
def process_ground_images(patch_size):
    for image_file in os.listdir(ground_images_dir):
        if image_file.endswith('.jpg'):
            ...
            # Save the cropped image in the aerial_image_patches_size directory
